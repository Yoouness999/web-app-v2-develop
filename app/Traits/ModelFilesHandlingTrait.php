...
trait ModelFilesHandlingTrait
{
    ...
    public function getCurrentPath()
    {
        ...
        if (!is_dir(static::$currentPath)) {
            try {
                if (!mkdir($concurrentDirectory = static::$currentPath, 0777, true) && !is_dir($concurrentDirectory)) {
        ...
        if ($params['path']) {
            try {
                if(!is_dir(public_path($data['path'].$params['path'])))
                    if (!mkdir($concurrentDirectory = public_path($data['path'] . $params['path']), 0777, true) && !is_dir($concurrentDirectory)) {
                        throw new \RuntimeException(sprintf('Directory "%s" was not created', $concurrentDirectory));
                    }
            } catch (Exception $e) {
                \Log::error($e);
