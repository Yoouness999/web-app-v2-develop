<?php
namespace App\Http\Controllers\Api\v2;

use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Api\v2\ApiHelper;
use App\Api\v2\ApiUnavailableDate;
use App\Api;

class ApiUnavailableDatesController extends Controller {
	/**
	 * Get unaivalables dates
	 *
	 * @param string $filters (optionnal) Filter the results. Model: {attribute1}:{operator1}:{value1};{attribute2}:{operator2}:{value2}...
	 * @param string $order (optionnal) Sort the result. Model: {attribute}:{way}
	 * @param int $page (optionnal) Current page for pagination.
	 * @param int $items_by_page (optionnal) Items by page for pagination.
	 * @param boolean $first (optionnal) Force the results to only one item. Not working with pagination.
	 */
	public function get(Request $request) {

		$data = [];
		if ($request->has('web')) {
			$data = Api::getUnavailableDates(null, $request->get('floor'), $request->get('volume'));
		} else {
			$params = ApiHelper::getParamsFromRequest($request);
			$data = ApiUnavailableDate::get($params);
		}
		return ApiHelper::response($data);
	}
	
	/**
	 * Add a unaivalable date.
	 *
	 * @param string $date  date. Format: YYYY-MM-DD HH:MM:SS.
	 */
	public function add(Request $request) {
		$item = ApiUnavailableDate::add($request->all());
		
		return ApiHelper::response($item);
	}
	
	/**
	 * Save an unaivalable date.
	 *
     * @param int $id  id of the unaivalable date.
     * @param string $date  date. Format: YYYY-MM-DD HH:MM:SS.
	 */
	public function save(Request $request) {
		$params = $request->all();
		$id = $params['id'];
		unset($params['id']);
		
		$item = ApiUnavailableDate::save($id, $params);
		
		return ApiHelper::response($item);
	}



    /**
     * Delete an unaivalable date.
     *
     * @param int $id (required) Id.
     */
    public function delete(Request $request) {
        $item = ApiUnavailableDate::delete($request->get('id'));

        return ApiHelper::response($item);
    }
}